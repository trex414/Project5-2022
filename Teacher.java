...
public class Teacher extends User {
    ...
    public Quiz addFileQuiz(String course,String fileName) {
        // lines will go
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(fileName))){
            for (int k = 0; k < courses.size(); k++) {
                if (course.equalsIgnoreCase(courses.get(k).getName())) {
                    ...
                    q = new Quiz(quizName, questions);
                    // courses.get(k).addQuiz(q);
        ...
        JOptionPane.showMessageDialog(null, "Quiz could not be created!", "Error", JOptionPane.ERROR_MESSAGE);
        return q;
    }
    
    // adds questions to a quiz and then adds that quiz to a course
    public Quiz addQuiz(String course, String quizName, ArrayList<String> questionsS, ArrayList<String> options,
        ...
        for (int k = 0; k < courses.size(); k++) {
            if (course.equalsIgnoreCase(courses.get(k).getName())) {
                for (int i = 0; i < questionsS.size(); i++) {
                    ...
                    questions.add(i, new Question(questionsS.get(i), optionsS.get(answers.get(i)), optionsS));
                }
                q = new Quiz(quizName, questions);
                //courses.get(k).addQuiz(q);
                //System.out.println("It worked!");
    ...
    public String modifyQuizName(String course, String quizName, String newQuizName, Manager m) {
            ...
            if (course.equalsIgnoreCase(cours.getName())) {
                for (int j = 0; j < cours.getQuizzes().size(); j++) {
                    if (quizName.equalsIgnoreCase(cours.getQuizzes().get(j).getName())) {
                            ...
                            m.getCourse(course).getQuiz(j).setName(newQuizName);
                            //System.out.println(newQuizName);
    ...
    public String modifyQuizPrompt(String course, String quizName, String question, String newprompt, Manager m) {
        courses = m.getCourses();
                    ...
                    if (quizName.equalsIgnoreCase(cours.getQuizzes().get(j).getName())) {
                        ...
                        for (int k = 0; k < cours.getQuizzes().get(j).getQuestions().size(); k++) {
                            if (question.equalsIgnoreCase
                                ...
                                m.getCourse(course).getQuiz(j).getQuestion(k).setPrompt(newprompt);
    ...
    public String modifyQuizChoices(String course, String quizName,String question, String choices, int answer, Manager m) {
                    ...
                    if (quizName.equalsIgnoreCase(cours.getQuizzes().get(j).getName())) {
                        ...
                        for (int k = 0; k < cours.getQuizzes().get(j).getQuestions().size(); k++) {
                            ...
                            if (question.equalsIgnoreCase
                                ...
                                m.getCourse(course).getQuiz(j).getQuestion(k).setChoices(optionsS);
                                modifyQuizAnswer(course, quizName, question, answer, m);
    ...
    public void modifyQuizAnswer(String course, String quizName, String question, int answer, Manager m) {
        for (Course cours : courses) {
            if (course.equalsIgnoreCase(cours.getName())) {
                // check for the quiz
                for (int j = 0; j < cours.getQuizzes().size(); j++) {
                    if (quizName.equalsIgnoreCase(cours.getQuizzes().get(j).getName())) {
                        ...
                        for (int k = 0; k < cours.getQuizzes().get(j).getQuestions().size(); k++) {
                            if (question.equalsIgnoreCase
                                ...
                                m.getCourse(course).getQuiz(j).getQuestion(k).setAnswer(m.getCourse(course).getQuiz(j).getQuestion(k).getChoice(answer));
                            }
                        }
                    }
                }
    ...
    public String removeQuiz(String course, String quizName, Manager m) {
        for (Course cours : courses) {
            if (course.equalsIgnoreCase(cours.getName())) {
                for (int j = 0; j < cours.getQuizzes().size(); j++) {
                    if (quizName.equalsIgnoreCase(cours.getQuizzes().get(j).getName())) {
                        cours.removeQuiz(cours.getQuizzes().get(j));
                        m.getCourse(course).removeQuiz(m.getCourse(course).getQuiz(j));
